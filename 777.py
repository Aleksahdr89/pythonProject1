...
for h in range(len(Sensor)):
    for k in range(len(Sensor[h])):
        if Sensor[h][k] == dimer_number:
            if (h+1) % 2 == 0:
                print((h + 1) // 2, 'ШКАФ - нижняя чать -', 'сверху в низ', k+1, 'автомат')
                print(((((h + 1) // 2) * 2) * 48) - (48 - (k+1)), '№ П/П , автомат из общего количества (480)')
            if (h+1) % 2 != 0:
                print(((h + 1) // 2)+1, 'ШКАФ - верхняя чать -', 'сверху в низ', k+1)
                print((((((h + 1) // 2)+1) * 2) * 48) - (48 + (k+1)), '№ П/П автомат из общего количества (480)')
#print(h+1, k+1)